...
**DISCLAIMER**. NCR is not an open source corpus, so we cannot provide it here. We demonstrate our handling of data, but the code in the **Setup** section is not executable autonomously. Jump to **Rhythm entropy** for fully reproducible analysis.
...
Build labels, define subset of meters, sample 700 poems per each meter; save.
...
-- do the stress annotation (`ru_accent_poet` module)
-- grab baseline prose at paragraphs level (sample 700)
-- construct quasi-poems (~700)
-- record text + binary
...
Sample consecutive paragraphs to the chunks of length of ~2000 words. 1 text = 1 chunk of 2k words.
...
```{r call python for stress, warning=F,message=F,eval=F}
...
p_lin + p_len
```
Split by words, calculate syllable cumulative sum (to chunk consecutively)
...
Each chunk of prose we cut to as many quasi poems as we can fit there; for each poem we determine lengths in lines and length of line drawing from RNC distribution.
...
So let's see what we get: this is one quasi poem of 14 lines cut approximately to 11 syllables per line (both dimension were determined randomly based on RNC distributions)---approximately, because it respects the words and does not cut through words.
...
**DISCLAIMER!** Code above should be autonomously executable, it has all data needed for this.
...
For classic meters that are governed by strong organizational principles, length of line barely has an influence on entropy scores. For free verse and quasi prose, entropy naturally increases with the increase in line length.
...
We see that modeling it as a monotonic effect does not bring much (model's predictions are a bit worse), so we will stick with categorical.
...
K-means clustering with the data for UMAP and for 5 basic meters separately
